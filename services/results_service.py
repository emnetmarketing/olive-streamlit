...
def save_analysis_result(result_data: dict[str, Any]) -> None:
    now = datetime.now(timezone.utc).isoformat()
    period = result_data.get("period", {})
    append_record("analysis_results", {
        "result_id": str(uuid4()), "created_at": now, "created_by": "shared_session",
    ...
    append_record("audit_logs", {"log_id": str(uuid4()), "created_at": now, "actor": "shared_session",
                                 "action": "analysis_saved", "target": "analysis_results",
                                 "details_json": json_text(result_data.get("metrics", {}))})
...
def recent_results(limit: int = 20) -> list[dict]:
    ...
    for item in reversed(records("analysis_results")):
        ...
        output.append({"id": item.get("result_id", ""), "created_at": item.get("created_at", ""),
                       "created_by": item.get("created_by", ""), "result_data": result_data})
